...
export function formatStellarError(err: unknown): StellarErrorInfo {
  const msg = (err as Error)?.message || String(err);

  if (/non-existent contract function|MissingValue/i.test(msg)) {
    return {
      message:
        "Contract mismatch: deployed contract does not expose init/purchase. Deploy this repo's contract and set VITE_CONTRACT_ID.",
      needsFunding: false,
      wrongContract: true,
    };
  }
  ...
  if (/already initialized|Error\(Contract, #1\)/i.test(msg)) {
    return {
      message: 'Supply hub is already initialized on this contract. You can proceed to purchase.',
      needsFunding: false,
    };
  }
  ...
  if (/not initialized|Error\(Contract, #2\)/i.test(msg)) {
    return {
      message: 'Supply hub has not been initialized yet. Click "Init Supply Hub" first.',
      needsFunding: false,
    };
  }
